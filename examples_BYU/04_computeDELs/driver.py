...
#TODO: make sure we use same turbine between part1 and part2
#TODO:
...
run_dir2            = run_dir1 + "/examples/01_aeroelasticse/" #os.path.dirname( os.path.realpath(__file__) ) + os.sep
...
TMax                    = 10.    # Length of wind grids and OpenFAST simulations, suggested 720 s
...
#    #Fixed-bottom
#    fastBatch.FAST_InputFile    = 'DTU10MW ----- IEA-10-240-RWT.fst'   # FAST input file (ext=.fst)
# ---> need to define it   fastBatch.FAST_directory    = os.path.join(run_dir2, 'OpenFAST_models','IEA-15-240-RWT','IEA-15-240-RWT')   # Path to fst directory files
...
B1MLx = np.zeros( (nnt,nnx) )

for i in range(nnx):
    tag = "B1N%03iMLx"%(i*dnx+1)
    B1MLx[:,i] = ct[0][tag][0:nnt*dnt-1:dnt]



# --------

print("Damage eq moment:")
print(DELs)
...
#
deB1MLx = .5 * ( fj * npDelstar[0,i_B1MLx] / n_life_eq ) ** (1/m) #as there is only one timeseries here, we do not have to sum over j...
...
## ------ simulate the damage constraint by specifying a dummy ultimate force ----

Fult = 2000

NeqLife = (Fult/deB1MLx)**m
D = n_life_eq / NeqLife
